...
export function ChatPanel() {
  ...
  const hasMessages = chat.messages.filter((m) => m.type !== 'system' || m.content.startsWith('📄') || m.content.startsWith('⚠') || m.content.startsWith('❌')).length > 0
      ...
      <div className="flex h-[42px] flex-shrink-0 items-center justify-end gap-1 border-b border-[#1a1a1a] bg-[#0d0d0d] px-3">
        <div
          className={`mr-1 h-1.5 w-1.5 rounded-full transition-all ${
            dotOk ? 'bg-accent-green shadow-[0_0_6px_theme(colors.accent.green)]' : dotErr ? 'bg-accent-red' : 'bg-[#2a2a2a]'
          }`}
        />
        <button
          onClick={clearChat}
              ...
              >
                <span
                  className={`h-[5px] w-[5px] flex-shrink-0 rounded-full transition-all ${
                    ok ? 'bg-accent-green shadow-[0_0_5px_theme(colors.accent.green)]' : err ? 'bg-accent-red' : 'bg-[#2a2a2a]'
                  }`}
                />
                {p}
              </button>
