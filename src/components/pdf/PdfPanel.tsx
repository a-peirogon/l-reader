...
export function PdfPanel() {
  ...
  // Native OS file picker — replaces the hidden <input type="file">
  const openNativeDialog = useCallback(async () => {
    try {
      const result = await invoke<[string, string] | null>('open_pdf_dialog')
      if (!result) return
      const [b64, name] = result
      const bytes = Uint8Array.from(atob(b64), c => c.charCodeAt(0))
      const file  = new File([bytes], name, { type: 'application/pdf' })
      loadFile(file)
    } catch (err) {
      console.error('[openNativeDialog]', err)
    }
  }, [loadFile])
  ...
  return (
    ...
    >
      ...
      {/* file-input-chat kept for compatibility with SearchPanel CustomEvent */}
