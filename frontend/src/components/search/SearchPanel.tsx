...
async function searchArxiv(q: string, limit = 12): Promise<SearchResult[]> {
  const res = await fetch(`/api/search/arxiv?q=${encodeURIComponent(q)}&limit=${limit}`)
  if (!res.ok) throw new Error('arXiv error')
    ...
    .map((r: any) => ({
      ...
      year: String(r.year ?? ''),
                      ...
                      abstract: r.abstract ?? '',
                      pdfUrl: r.pdfUrl ?? null,         // comes pre-built from backend
                      link: r.url ?? '',                // backend field is "url", not "link"
    }))
    .sort((a: any, b: any) => (b.year || 0) - (a.year || 0))  // newest first
}

async function searchScholar(q: string, limit = 12): Promise<SearchResult[]> {
  const res = await fetch(`/api/search/scholar?q=${encodeURIComponent(q)}&limit=${limit}`)
  if (!res.ok) throw new Error('Scholar error')
    const data = await res.json()
    return (data.results ?? [])
    .map((r: any) => ({
      id: r.id,
      ...
      authors: (r.authors ?? []).map((a: any) => a.name ?? a),
                      year: String(r.year ?? ''),
                      abstract: r.abstract ?? '',
                      pdfUrl: r.pdfUrl ?? null,         // backend field is "pdfUrl"
                      link: r.url ?? '',                // backend field is "url"
    }))
...
function ResultCard({ result, onImport }: { result: SearchResult; onImport: (url: string) => void }) {
  ...
  const handleImport = async () => {
    if (!result.pdfUrl) return
      setImporting(true)
      try {
        const res = await fetch(`/api/pdf/fetch?url=${encodeURIComponent(result.pdfUrl)}`)
        if (!res.ok) throw new Error('fetch failed')
          const blob = await res.blob()
          const file = new File([blob], result.title.slice(0, 60) + '.pdf', { type: 'application/pdf' })
