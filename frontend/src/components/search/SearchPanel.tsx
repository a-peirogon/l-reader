...
function ResultCard({ result, onImport, importing }: { result: SearchResult; onImport: (r: SearchResult) => void; importing?: boolean }) {
  ...
  return (
    ...
    {result.pdfUrl && (
      ...
      disabled={importing}
      title="Abrir en Lectio"
      className="flex h-[20px] w-[20px] items-center justify-center rounded text-[#333] transition-colors hover:bg-[#1a1a1a] hover:text-[#bbb] disabled:opacity-40"
      >
      {importing ? (
...
export function SearchPanel({ onClose }: SearchPanelProps) {
  ...
  const [importing, setImporting] = useState<string | null>(null) // result.id being imported
  ...
  // ── Import PDF via backend proxy ────────────────────────────────────────────
  const importPdf = useCallback(async (result: SearchResult) => {
    if (!result.pdfUrl || importing) return
      setImporting(result.id)

      const BASE = (import.meta as any).env?.VITE_API_URL ?? ''
  const proxyUrl = `${BASE}/api/pdf/fetch?url=${encodeURIComponent(result.pdfUrl)}`
  ...
  try {
    ...
    ? result.title.slice(0, 60).replace(/[^a-zA-Z0-9 _-]/g, '') + '.pdf'
  : (result.pdfUrl.split('/').pop() ?? 'paper.pdf')

  const file = new File([blob], filename, { type: 'application/pdf' })

  // Dispatch a synthetic change event on the shared file input so
  // PdfPanel's handleFile() picks it up — same path as manual open.
  const input = document.getElementById('file-input-pdf') as HTMLInputElement | null
  if (input) {
  ...
  onClose()
    ...
    alert(`No se pudo importar el PDF: ${err.message}`)
  } finally {
    setImporting(null)
  }
  }, [importing, onClose])
